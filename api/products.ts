...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  if (req.method === 'PUT' && id) {
    const { number, title, description, sub, price, img, img_position, img_left } = req.body
    await sql`UPDATE products SET number=${number}, title=${title}, description=${description}, sub=${sub}, price=${price}, img=${img}, img_position=${img_position}, img_left=${img_left} WHERE id=${id}`
